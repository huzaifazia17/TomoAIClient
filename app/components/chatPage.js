...
export default function ChatPage({ currentSpaceId, currentChatId, updateChatMessages }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { contextSummary, response } = aiResponse; // Destructure response
  
      // Format the AI message with both context and response
      const aiMessage = {
        role: 'ai',
        content: {
          contextSummary, // Add context summary
          response, // Add AI response
        },
      };
  
      // Update chat messages (this will display the response in the appropriate section)
      updateChatMessages(currentChatId, aiMessage);
  ...
  //   return (
  //     <div
  //       className={`message ${message.role} p-4 ${
  //         message.role === 'ai' ? 'bg-gray-800 text-white' : 'bg-gray-200 text-black'
  //       } rounded-md`}
  //     >
